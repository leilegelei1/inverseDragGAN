...
parser.add_argument('--cache-dir', type=str, default='./DragGAN/checkpoints')
...
with gr.Blocks() as app:
    ...
    global_state = gr.State({
        "images": {
            ...
            'image_upload':PIL.Image.fromarray((np.ones([512,512,3],dtype=np.uint8)*255))
        },
        "temporal_params": {
            # stop
        },
        ...
        with gr.Row():
            ...
            with gr.Column(scale=3):
                ...
                with gr.Column(scale=5):
                    with gr.Row():
                        form_image_upload = ImageMask(
                            value=global_state.value['images']['image_upload'],
                            brush_radius=20).style(
                            width=512,
                            height=512)
                    with gr.Row():
                        form_upload_image_btn = gr.UploadButton("Upload Image",file_types=['image'])
                    with gr.Row():
                        with gr.Column(scale=1,min_width=10):
                            form_start_embedding_btn = gr.Button("Start Embedding")
                        with gr.Column(scale=1,min_width=10):
                            form_step_embedding_btn = gr.Button("Stop Embedding")
    ...
    def upload_img(file_p,global_state):
        img = PIL.Image.open(file_p.name)
        img = img.resize((512,512))
        global_state['images']['image_upload'] = img
        return global_state,global_state['images']['image_upload']
    form_upload_image_btn.upload(
        upload_img,
        inputs=[form_upload_image_btn,global_state],
        outputs=[global_state,form_image_upload]
    )

    #embed img into embedding
    def on_click_start_embed(global_state,form_image_upload):
        global_state["temporal_params_embed"]["step"] = False
        w = torch.ones([1,512],device=device,requires_grad=True) * 0.3
        generator = global_state["renderer"].G
        label = torch.zeros([1,generator.c_dim]).to(device)

        w_optim = torch.optim.Adam([w],lr=0.001)
        change = True

        img_tensor = torch.from_numpy(np.array(form_image_upload[()]['image'])).permute([2,0,1])[None].to(device)
        img_tensor = (img_tensor - 128.) / 127.5

        from vgg_loss import VGGLoss
        vgg_loss_ = VGGLoss().to(device)
        target_feats = vgg_loss_.get_features((img_tensor+1.)/2.)
        for i in range(1000):
            if i > 500 and change:
                params = [w]
                for k,v in generator.named_parameters():
                    params.append(v)
                w_optim = torch.optim.Adam(params,lr=0.0001)
                change = False

            if global_state["temporal_params_embed"]["stop"]:
                net_weights = generator.state_dict()
                init_images(global_state)
                break

            ws = generator.mapping(w,label,truncation_psi=global_state["params"]["trunc_psi"],tranction_cutoff=global_state["params"]["trunc_cutoff"])
            img,feat = generator(ws,label,truncation_psi=global_state["params"]["trunc_psi"],noise_mode='const',
                                 input_is_w=True,return_features=True)
            l1_loss = torch.nn.functional.mse_loss(img,img_tensor)
            perceptual_loss = vgg_loss_((img+1.)/2.,target_feats)
            ...
            img_np = (img[0] * 127.5 + 128).clamp(0,255).to(torch.uint8).permute(1,2,0).cpu().numpy()
            global_state["images"]["image_show"] = PIL.Image.fromarray(img_np)
            global_state["images"]["image_raw"] = PIL.Image.fromarray(img_np)
            global_state["load_w"] = ws.detach()
    ...
    form_start_embedding_btn.click(
        on_click_start_embed,
        inputs=[global_state,form_image_upload],
        outputs=[global_state,form_image]
    )

    def on_click_stop_embed(global_state):
        global_state["temporal_params_embed"]["stop"] = True
        return global_state
    form_step_embedding_btn.click(
        on_click_stop_embed,
        inputs=[global_state],
